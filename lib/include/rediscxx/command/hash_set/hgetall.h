...
namespace rediscxx {
    class hgetall_command : public command_base, public core::ref_counted<hgetall_command> {
        using on_handler = std::function<void(std::expected<result::reply, error::redis_exception>)>;
    public:
        COMMAND_CLASS_TYPE(hgetall);
        explicit hgetall_command(internal::arg_buffer&& args, on_handler&& handler)
        : m_args(std::move(args)), m_handler(std::move(handler)) {
            m_args.append_at_front(hgetall_command::to_string());
        }
    public:
    ...
    private:
        static void on_handle(redisAsyncContext* ctx, void* r, void* priv) noexcept {
            const auto self = std::move(*static_cast<smart_ptr::intrusive_ptr<hgetall_command>*>(priv));
            delete static_cast<smart_ptr::intrusive_ptr<hgetall_command>*>(priv);
            using error::types::HGetAllCommand;
            if (const auto* reply = static_cast<redisReply*>(r); !reply) {
                self->m_handler(std::unexpected(error::from_ctx(ctx)));
            }
            else if (reply->type == REDIS_REPLY_ERROR) {
                self->m_handler(MAKE_UNEXPECTED_ERROR(error::redis_exception, HGetAllCommand, reply->str));
            }
            else {
