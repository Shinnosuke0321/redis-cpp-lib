...
#define COMMAND_CLASS_TYPE(cmd) \
    ...
    public:
        explicit auth_command(std::string password, std::function<void(std::expected<void, core::connection_error>)>&& handler)
    ...
    public:
        ...
        void execute(redisAsyncContext *ctx) override {
            if (auto self = this->intrusive_from_this();
                ...
                m_handler(std::unexpected(CONNECTION_ERROR(AuthFailed, "failed to send auth")));
            }
        }
    ...
    private:
        static void on_callback(redisAsyncContext *ctx, void *r, void *priv) {
            ...
            if (const auto *reply = static_cast<redisReply*>(r); !reply) {
                self->m_handler(std::unexpected(CONNECTION_ERROR(ConnectionFailed, "reply is null")));
            }
            else if (reply->type == REDIS_REPLY_ERROR) {
                self->m_handler(std::unexpected(CONNECTION_ERROR(ConnectionFailed, reply->str)));
            }
            else {
                self->m_handler({});
            }
    ...
    private:
        ...
        std::function<void(std::expected<void, core::connection_error>)> m_handler;
    };

    class select_command: public command_base, public core::ref_counted<select_command> {
    public:
        explicit select_command(const int index, std::function<void(std::expected<void, core::connection_error>)>&& handler)
        : m_index(index),
          m_handler(std::move(handler)){}
    public:
        ...
        void execute(redisAsyncContext *ctx) override {
            if (auto self = this->intrusive_from_this();
                ...
                m_handler(std::unexpected(CONNECTION_ERROR(ConnectionFailed, "failed to send select")));
            }
        }
