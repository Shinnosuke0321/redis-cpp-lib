...
namespace rediscxx {
    class flush_db_command : public command_base, public core::ref_counted<flush_db_command> {
    public:
    ...
    public:
        explicit flush_db_command(internal::arg_buffer&& args, on_callback&& handler)
        : m_args(std::move(args)), m_handler(std::move(handler)) {
            m_args.append_at_front(flush_db_command::to_string());
        }
    ...
    private:
        static void on_handle(redisAsyncContext* ctx, void* r, void* priv) noexcept {
            const auto self = std::move(*static_cast<smart_ptr::intrusive_ptr<flush_db_command>*>(priv));
            delete static_cast<smart_ptr::intrusive_ptr<flush_db_command>*>(priv);
            using error::types::FlushDbCommand;
            if (const auto* reply = static_cast<redisReply*>(r); !reply) {
                self->m_handler(std::unexpected(error::from_ctx(ctx)));
            }
            else if (reply->type == REDIS_REPLY_ERROR) {
                self->m_handler(MAKE_UNEXPECTED_ERROR(error::redis_exception, FlushDbCommand, reply->str));
            }
            else {
