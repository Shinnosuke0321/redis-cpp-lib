...
namespace rediscxx {
    class smembers_command: public command_base, public core::ref_counted<smembers_command> {
    public:
        using on_callback = std::function<void(std::expected<result::reply, error::redis_exception>)>;
    public:
        COMMAND_CLASS_TYPE(smembers)

        explicit smembers_command(internal::arg_buffer&& args, on_callback&& handler)
        : m_args(std::move(args)), m_handler(std::move(handler)) {
            m_args.append_at_front(smembers_command::to_string());
        }
    ...
    private:
        static void on_handle(redisAsyncContext *ctx, void *r, void *priv) noexcept {
            const auto self = std::move(*static_cast<smart_ptr::intrusive_ptr<smembers_command>*>(priv));
            delete static_cast<smart_ptr::intrusive_ptr<smembers_command>*>(priv);
